import axios from "axios";
import React, { useEffect, useReducer } from "react";
import { Button, Container, Table } from "react-bootstrap";
import { Link } from "react-router-dom";

const Read = () => {
  const second = {
    loding: true,
    post: {},
    error: "",
  };

  const first = (state, action) => {
    switch (action.type) {
      case "pass":
        return {
          loding: false,
          post: action.answer,
          error: "",
        };
      case "fail":
        return {
          loding: false,
          post: {},
          error: "somenting went worng!!!...",
        };
      default:
        return state;
    }
  };

  const [state, dispatch] = useReducer(first, second);
  const createData = () => {
    axios
      .get("http://localhost:8000/employees") //my API
      .then((res) => {
        // console.log(res);
        dispatch({
          type: "pass",
          error: "",
          answer: res.data,
        });
      })
      .catch((error) => {
        dispatch({
          type: "fail",
          error: "",
          post: {},
        });
      });
  };

  useEffect(() => {
    createData();
  }, []); //no dependency...

  const deleted = (mapapi) => {
    axios
      .delete(`http://localhost:8000/employees/${mapapi}`)
      .then((deletedata) => {
        // console.log('deleted success')
        createData();
      })
      .catch((error) => {
        console.log(error);
      });
  };
  return (
    <div className="read">
      <h1 style={{ textAlign: "center" }}> Welcome to My Crud operation</h1>
      <Container>
        <Table className="striped-col">
          <thead>
            <tr>
              <td>ID</td>
              <td>NAME</td>
              <td>AGE</td>
              <td>MAIL ID</td>
              <td>DESIGINATION</td>
              <td>EMPLOYEE ID</td>
              <td>EDIT</td>
              <td>DELETE</td>
            </tr>
          </thead>
          {state.post.length > 0 &&
            state.post.map((mapapi) => {
              return (
                <tbody className="dark">
                  <tr key={mapapi.id}>
                    <td>{mapapi.id}</td>
                    <td>{mapapi.name}</td>
                    <td>{mapapi.age}</td>
                    <td>{mapapi.email}</td>
                    <td>{mapapi.desi}</td>
                    <td>{mapapi.employeeid}</td>
                    <td>
                      <Link to={`/update/${mapapi.id}`}>
                        <Button variant="outline-secondary" size="lg">
                          
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="16"
                            height="16"
                            fill="currentColor"
                            class="bi bi-pencil-square"
                            viewBox="0 0 16 16"
                          >
                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                            <path
                              fill-rule="evenodd"
                              d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"
                            />
                          </svg>
                        </Button>
                      </Link>
                    </td>
                    <td>
                      <Button
                        onClick={() => deleted(mapapi.id)}
                        variant="outline-danger"
                        size="lg"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="16"
                          height="16"
                          fill="currentColor"
                          class="bi bi-trash-fill"
                          viewBox="0 0 16 16"
                        >
                          <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z" />
                        </svg>
                      </Button>
                    </td>
                  </tr>
                </tbody>
              );
            })}
        </Table>
        <Link to="/api">
          <Button variant="outline-warning col-6">C for (create) </Button>
        </Link>
        <Link to="/form">
          <Button variant="outline-success col-6">R for (read) </Button>
        </Link>
      </Container>
      <br />
      <marquee>
        <b>THANK YOU VISIT AGAIN </b> &#128516;&#128516;&#128516; .
      </marquee>
    </div>
  );
};

export default Read;
